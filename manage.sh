...
healthcheck() {
    ...
    for i in $(seq 1 $max_attempts); do
        HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://localhost:3400 2>/dev/null || echo "000")

        if [ "$HTTP_STATUS" = "200" ] || [ "$HTTP_STATUS" = "302" ] || [ "$HTTP_STATUS" = "307" ]; then
            log_success "애플리케이션이 정상 응답합니다. (HTTP $HTTP_STATUS)"
            log_info "접속: http://localhost:3400"
            return 0
        fi
...
shell() {
    ...
    docker-compose exec bus-finder /bin/sh
}
...
ghcr_build() {
    ...
    docker buildx build \
        ...
        --build-arg NEXT_PUBLIC_PUBLICDATA_API_KEY="$NEXT_PUBLIC_PUBLICDATA_API_KEY" \
        -t "ghcr.io/${username}/bus_finder:${tag}" \
        --push \
        "$APP_DIR"

    if [ $? -eq 0 ]; then
        log_success "빌드 및 푸시 완료"
        log_info "이미지: ghcr.io/${username}/bus_finder:${tag}"
    else
        log_error "빌드 실패"
        exit 1
    fi
